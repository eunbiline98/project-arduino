...
DMD dmd(3, 1); //untuk mengatur jumlah panel yang kamu pakai
  ...
  if (now.second() > 1) {
    ...
    if (now.hour() == hours && now.minute() == minutes && now.second() > 1 ) {
      ...
      for (lamanya = 0; lamanya < 150; lamanya++) {
        ...
        dmd.drawString(33, 0, tampil, pj, 0); //koordinat tampilan
        ...
        dmd.drawString(33, 9, waktu_iqomahx, panjang_iqomahx, 0);
      ...
      dmd.clearScreen( true );
      dmd.clearScreen( true );
      dmd.drawString(30, 0, "IQOMAH", 6, 0); //koordinat tampilan
      int tampil;
      ...
      for (detik = 60; detik >= 0; detik--) {
        ...
        dmd.drawString(33, 9, waktu_iqomah, panjang_iqomah, 0);
        if (detik <= 0) {
          detik = 60;
          menit = menit - 1;
        }
        ...
        if (menit <= 0 && detik <= 1) {
          ...
          for (tampil = 0; tampil < 500 ; tampil++) {
            menit = 0;
            detik = 0;
            dmd.drawString(29, 0, "SHOLAT", 6, 0); //koordinat tampilan
            ...
            dmd.drawString(25, 9, sjam, pjg, 0);
    ...
    if (now.hour() == hours && now.minute() == minutes && now.second() > 1 ) {
      ...
      for (lamanya = 0; lamanya < 150; lamanya++) {
        ...
        Teks.toCharArray(tampil, pj);
        dmd.drawString(33, 0, tampil, pj, 0); //koordinat tampilan
        String iqomahx = "MAGHRIB" ;
        int panjang_iqomahx = iqomahx.length() + 1;
        char waktu_iqomahx[panjang_iqomahx];
        iqomahx.toCharArray(waktu_iqomahx, panjang_iqomahx);
        dmd.drawString(29, 9, waktu_iqomahx, panjang_iqomahx, 0);
    ...
    if (now.hour() == hours && now.minute() == minutes && now.second() > 1) {
      ...
      for (lamanya = 0; lamanya < 150; lamanya++) {
        ...
        char waktu_iqomahx[panjang_iqomahx];
        iqomahx.toCharArray(waktu_iqomahx, panjang_iqomahx);
        dmd.drawString(35, 9, waktu_iqomahx, panjang_iqomahx, 0);
...

    {
      ...
      for (lamanya = 0; lamanya < 2700; lamanya++) {
        lamanya = lamanya + 1;
        delay(1000);
        dmd.drawString(25, 0, "MATIKAN HP", 10, 0); //koordinat tampilan
        digitalWrite(3, HIGH);//matikan relay power TF-SW
        ///////////////////////
        now = rtc.now();
        String xjam = Konversi(now.hour()) + ":" + Konversi(now.minute()) + ":" + Konversi(now.second()) ; //tampilan jam
    ...
    if (now.hour() == hours && now.minute() == minutes && now.second() > 5 ) {
      digitalWrite(2, HIGH);//alarm sholat
      delay(1000);
      digitalWrite(2, LOW);//alarm sholat
      delay(500);
...
////
 ...
 String hariku1 = weekDay[now.dayOfWeek()] ;
    ...
    char detikanku1[dowo1];
    hariku1.toCharArray(detikanku1, dowo1);
    dmd.drawString(47, 0, detikanku1, dowo1, 0);
 ...
 ///////////////interupsi tombol menu-----------
     ...
     String hariku2 = Konversi(now.date()) + "." + Konversi(now.month()) + "." + Konversi(now.year()) ;
    int dowo2 = hariku2.length() + 1;
    char detikanku2[dowo2];
    hariku2.toCharArray(detikanku2, dowo2);
    dmd.drawString(33, 9, detikanku2, dowo2, 0);
...
///
 ...
 String hariku = Konversi(now.date()) + "." + Konversi(now.month()) + "." + Konversi(now.year()) ;
    int dowo = hariku.length() + 1;
    char detikanku[dowo];
    hariku.toCharArray(detikanku, dowo);
    dmd.drawString(35, 0, detikanku, dowo, 0);
...
////
    ...
    Teks = "  IMSAK:" + Konversi(hours0) + ":" + Konversi(minutes0) + "  SUBUH:" + Konversi(hours1) + ":" + Konversi(minutes1)
    ...
    int kecepatan;
    kecepatan = 90;     //kecepatan runing teks
    int pj = Teks.length() + 1;
    char tampil[pj];
    Teks.toCharArray(tampil, pj);
    dmd.selectFont(SystemFont5x7);
    dmd.drawMarquee(tampil, pj, 96, 9);
 ///////////////interupsi tombol menu-----------
  ...
  val; //pencacah nilai
    ...
    while (!ret) {
      if ((timer + kecepatan) < millis()) {
        ret = dmd.stepSplitMarquee(8, 15, 32); ///dianimasikan baris 8 sampai 15, sampai kolom 0
        timer = millis();
